  ...
  try{
    ...
    async function doResumePaused(id){
      try{
        ...
        try { cart = rows.map(i=>({ item_code:i.item_code, item_name:i.item_name, qty:Number(i.qty||0), rate:Number(i.rate||0), amount:Number(i.qty||0)*Number(i.rate||0), refund: !!i.refund })); } catch(_){ window.cart = rows; }
